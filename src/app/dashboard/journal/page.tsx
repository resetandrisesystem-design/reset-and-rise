  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const [{ data: entry }, { data: past }] = await Promise.all([
        ...
        supabase.from("journal_entries").select("*").eq("user_id", session.user.id).neq("entry_date", today).order("entry_date", { ascending: false }).limit(50),
      ]);

      setData({ userId: session.user.id, today, initialEntry: entry, pastEntries: past ?? [] });
    });
  }, []);
